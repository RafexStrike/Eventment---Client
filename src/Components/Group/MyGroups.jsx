...
const MyGroups = () => {
  ...
  // console.log(allGroupsFetchedFromDB)

  const myOwnCreatedGroups = allGroupsFetchedFromDB.filter(
    (myCreatedGroup) => myCreatedGroup.email == user.email
  );
  console.log(myOwnCreatedGroups);

  const [initialGroups, setInitialGroups] = useState(myOwnCreatedGroups)
      ...
      <div className="max-w-6xl mx-auto my-6">
        <div className="card bg-base-100 shadow-lg">
          <div className="card-body text-center">
            ...
            <h2 className="card-title text-2xl mb-2 justify-center">No Groups Created Yet</h2>
            <p className="text-base-content/70 mb-4">
              You haven't created any groups yet. Start building your community by creating your first group!
            </p>
            <div className="card-actions justify-center">
              <Link to={`/createGroup`} className="btn btn-primary">Create Your First Group</Link>
            </div>
          </div>
        </div>
      </div>
